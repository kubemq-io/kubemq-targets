...
func TestClient_Init(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "init",
			cfg: config.Spec{
				Name: "cockroachdb-target",
				Kind: "cockroachdb.target",
		...
		},
		{
			...
			cfg: config.Spec{
				...
				Kind: "cockroachdb.target",
				Properties: map[string]string{
					"connection":                      "postgres://root:postsgres@localhost:4000/postgres?sslmode=disable",
					"max_idle_connections":            "",
					"max_open_connections":            "",
					"connection_max_lifetime_seconds": "",
				},
	...
	tests := []struct {
	...
	}{
		{
			...
			request: types.NewRequest().
				SetMetadataKeyValue("method", "exec").
				SetData([]byte("DROP TABLE IF EXISTS post;")),
			wantErr: false,
		},
		{
